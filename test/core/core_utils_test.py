...
@pytest.mark.parametrize(
    "period_str, expected_period_mode, expected_period_time",
    [
...
@pytest.mark.parametrize(
    "num_str, positive_only, expected_num, expected_error",
    [
...
@pytest.mark.parametrize(
    "substr, texts, lower_case, expected",
    [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ['"first', 'quoted"', "middle", '"second', 'quoted"'], ["first quoted", "middle", "second quoted"], id="multiple_quoted"
        ),
...
@pytest.mark.parametrize(
    "word, list_of_multiple_words, expected",
    [
...
@pytest.mark.parametrize(
    "date_str, should_succeed, expected_format",
    [
...
@pytest.mark.parametrize(
    "date_range_str, should_succeed, expected_format",
    [
...
@pytest.mark.parametrize(
    "text, min_len, max_len, should_succeed, has_separator",
    [
...
@pytest.mark.parametrize(
    "period_mode, period_time, dt, start_dt, end_dt, dt_format, expected",
    [
...
def test_generate_period_headline_date():
    ...
    command_args = CommandArgs(period_mode=PeriodFilterMode.DATE, dt=TEST_DT_2024_01_15, dt_format=DatetimeFormat.DATE)
    result = generate_period_headline(command_args)
    assert "15-01-2024" in result
...
def test_generate_period_headline_date_range():
    ...
    command_args = CommandArgs(
        period_mode=PeriodFilterMode.DATE_RANGE, start_dt=TEST_DT_2024_01_01, end_dt=TEST_DT_2024_01_31, dt_format=DatetimeFormat.DATE
    )
    result = generate_period_headline(command_args)
...
@pytest.mark.parametrize(
    "user_str, should_succeed, expected_user, expected_user_id",
    [
...
def test_is_named_arg(arg, expected):
    ...
    command_args = CommandArgs(
        available_named_args={"user": ArgType.USER, "period": ArgType.PERIOD}, available_named_args_aliases={"u": "user", "p": "period"}
    )
...
@pytest.mark.parametrize(
    "num_str, min_num, max_num, positive_only, should_succeed, error_contains",
    [
...
@pytest.mark.parametrize(
    "message_id, message_type, expected_extension",
    [
