...
@pytest.mark.parametrize("words_str, ratio_threshold, stopwords, expected", [
...
@pytest.mark.parametrize("words_str, expected", [
...
@pytest.mark.parametrize("start_dt, end_dt, expected_count", [
    pytest.param(datetime(2023, 10, 9, 0, 0, 0).replace(tzinfo=ZoneInfo(TIMEZONE)),
        datetime(2023, 10, 10, 0, 0, 0).replace(tzinfo=ZoneInfo(TIMEZONE)), 2, id="one_day_range"
    ),
    pytest.param(datetime(2023, 1, 1, 0, 0, 0).replace(tzinfo=ZoneInfo(TIMEZONE)),
                 datetime(2024, 1, 1, 0, 0, 0).replace(tzinfo=ZoneInfo(TIMEZONE)), 8, id="full_year"),
    pytest.param(datetime(2025, 1, 1, 0, 0, 0).replace(tzinfo=ZoneInfo(TIMEZONE)),
        datetime(2026, 1, 1, 0, 0, 0).replace(tzinfo=ZoneInfo(TIMEZONE)), 0, id="future_range"),
])
...
@pytest.mark.parametrize("emoji_type, expected_non_empty", [
...
@pytest.mark.parametrize("emoji_type, expected_count", [
...
@pytest.mark.parametrize("new_username, should_succeed", [
...
@pytest.mark.parametrize("text, characters_filter, expected", [
...
@pytest.mark.parametrize("extension, expected_pattern", [
...
@pytest.mark.parametrize("user_id, expected_message_id, should_succeed", [
