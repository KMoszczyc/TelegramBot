...
def test_easiness_score_multipliers(mock_countries_df):
    countries = Countries(data=mock_countries_df)
    df = countries.df

    polska_row = df[df["country_name"] == "Polska"].iloc[0]
    # Polska (EU) pop: 38,000,000 * 50.0 / 100,000 = 19,000.0
    assert polska_row["easiness_score"] == pytest.approx(19000.0)

    egipt_row = df[df["country_name"] == "Egipt"].iloc[0]
    # Egipt (AF) pop: 104,000,000 * 0.1 / 100,000 = 104.0
    assert egipt_row["easiness_score"] == pytest.approx(104.0)

    chiny_row = df[df["country_name"] == "Chiny"].iloc[0]
    # Chiny (AS) pop: 1,400,000,000 * 1.0 / 100,000 = 14,000.0
