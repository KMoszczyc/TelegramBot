...
def create_table_plotly(df, command_args, columns):
    ...
    CELL_WIDTH = 280
    WIDTH = (
        CELL_WIDTH * 1.2 + CELL_WIDTH * (len(columns) - 1)
        if is_date_range or is_date
        else CELL_WIDTH * 0.9 + CELL_WIDTH * (len(columns) - 1)
    )
    WIDTHS = (
        [CELL_WIDTH * 1.2] + [CELL_WIDTH] * (len(columns) - 1)
        if is_date_range or is_date
    ...
    layout = go.Layout(
        autosize=True, margin={"l": 0, "r": 1, "t": 0, "b": 0}, height=CELL_HEIGHT * len(df) + HEADER_CELL_HEIGHT + 1, width=WIDTH
    )
...
def create_relationship_graph(reactions_df, col_1, col_2):
    min_node_size = 300
    max_node_size = 10000
    min_edge_width = 1
    max_edge_width = 30
    ...
    # Draw nodes
    nx.draw_networkx_nodes(G, pos, node_color="#e68a00", node_size=node_sizes, alpha=0.95, ax=ax)

    # Draw edges
    nx.draw_networkx_edges(G, pos, width=edge_widths, edge_color="#ffaa00", alpha=0.6, ax=ax)
...
def create_bidirectional_relationship_graph(reactions_df, col_1, col_2, graph_label):
    ...
    for (u, v), w in zip(G.edges(), edge_widths, strict=False):
        x1, y1 = pos[u]
        x2, y2 = pos[v]
        ...
        if sign != 0:
            if abs(dx) >= abs(dy):
                oy = EDGE_OFFSET * sign  # horizontal → split vertically
            else:
                ox = EDGE_OFFSET * sign  # vertical → split horizontally

        ax.plot([x1 + ox, x2 + ox], [y1 + oy, y2 + oy], linewidth=w, color="#ffaa00", alpha=0.6, zorder=1)

    # # Draw node labels (slightly above nodes)
    for node, (x, y) in pos.items():
        # Get the degree or total weight of the node (number of reactions)w
        reaction_count = sum(d["weight"] for _, _, d in G.in_edges(node, data=True)) + sum(
    ...
    for u, v, d in G.edges(data=True):
        ...
        # # offset labels so they are not in the middle
        # t = 0.5 + 0.15 * sign # 0.4 or 0.6
        # mx = x1 + dx * t + ox
        # my = y1 + dy * t + oy
        ...
        # choose position along canonical edge
        t = 0.45 if (u, v) == (a, b) else 0.55
        mx = xa + (xb - xa) * t + ox
        my = ya + (yb - ya) * t + oy
