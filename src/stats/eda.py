...
def generate_word_stats():
    ...
    word_counts = dict(zip(word_list, counts, strict=False))
    ...
    cleaned_word_counts = [
        (words_str, count)
        for words_str, count in sorted_word_counts
        if not stats_utils.contains_stopwords(words_str, polish_stopwords) and stats_utils.is_ngram_valid(words_str)
    ]  # bigrams/trigrams
    kinda_cleaned_word_counts = [
        (words_str, count)
        for words_str, count in sorted_word_counts
        if (not stats_utils.is_ngram_contaminated_by_stopwords(words_str, STOPWORD_RATIO_THRESHOLD, polish_stopwords))
        and stats_utils.is_ngram_valid(words_str)
    ]  # bigrams/trigrams
