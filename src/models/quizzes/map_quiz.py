...
class MapQuiz(BaseGuessQuiz):
    ...
    def get_display_name(self, item: dict | pd.Series) -> str:
        person = item if isinstance(item, dict) else item.to_dict()
        is_polish = False
    ...
    def get_description(self, item: dict | pd.Series, max_length: int = 3500, extended: bool = False) -> str:
        person = item if isinstance(item, dict) else item.to_dict()
    ...
    def get_tips(self, item: dict | pd.Series, continent_specified: bool = False) -> list[str]:
        ...
        valid_answers = self.get_valid_answers(person)
    ...
    def guess_random_person_on_map(self, people_trivia_df):
        ...
        image_path = self.get_image_path(person)
